...
class GraphIndependent(nn.Module):
  ...
  def __init__(self,
    ...
    if edge_model_fn is None:
      ...
      self._edge_model = lambda x: edge_model_fn()(x)  # pylint: disable=unnecessary-lambda
    if node_model_fn is None:
      self._node_model = lambda x: x
    else:
      self._node_model = lambda x: node_model_fn()(x)  # pylint: disable=unnecessary-lambda
    if global_model_fn is None:
      self._global_model = lambda x: x
    else:
      self._global_model = lambda x: global_model_fn()(x)  # pylint: disable=unnecessary-lambda
...
def _received_edges_normalizer(graph,
  ...
  return normalizer(
      ...
      num_segments=torch.sum(graph.n_node))
...
class SelfAttention(nn.Module):
  ...
  def forward(self, node_values, node_keys, node_queries, attention_graph):
    ...
    received_edges_aggregator = blocks_torch.ReceivedEdgesToNodesAggregator(
        reducer=scatter_add)
